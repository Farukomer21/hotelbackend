...
const { ReservationRetrivedPublisher } = require("../../api-events/publishers");
...
const { dbScriptGetReservation } = require("dbLayer");

class GetReservationByIdManager extends ReservationManager {
  ...
  async doBusiness() {
    const reservation = await dbScriptGetReservation(this);
    return reservation;
  }
  ...
  async raiseEvent() {
    ReservationRetrivedPublisher.Publish(this.output, this.session).catch(
      (err) => {
...
module.exports = GetReservationByIdManager;
