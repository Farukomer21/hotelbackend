...
class CancelReservationByCodeMcpController extends ReservationManagementMcpController {
  constructor(params) {
    super("cancelReservationByCode", "cancelreservationbycode", params);
    this.dataName = "reservation";
    this.crudType = "update";
  }

  createApiManager() {
    return new CancelReservationByCodeManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      checkInDate: z.string().optional().describe("Reservation check-in date."),

      checkOutDate: z
        .string()
        .optional()
        .describe("Reservation check-out date."),

      status: z
        .enum([])
        .optional()
        .describe("Status of the reservation: booked, canceled, or completed."),
      ...
      notes: z
        .string()
        .optional()
        .describe("Additional notes by staff or guest for this reservation."),
...
module.exports = (headers) => {
  return {
    ...
    parameters: CancelReservationByCodeMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new CancelReservationByCodeMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return CancelReservationByCodeMcpController.getOutputSchema().parse(result);
