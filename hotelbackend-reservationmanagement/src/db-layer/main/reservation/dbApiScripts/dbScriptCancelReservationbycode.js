...
// This script is called from the CancelReservationByCodeManager
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.reservation;
  return {
    old_reservation: oldData,
    reservation: newData,
...
const dbScriptCancelReservationbycode = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Reservation.update(dataClause, {
      where: whereClause,
      returning: true,
    });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptCancelReservationbycode",
      {
...
module.exports = dbScriptCancelReservationbycode;
