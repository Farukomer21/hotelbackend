...
// This script is called from the DeleteReservationManager
...
const dbScriptDeleteReservation = async (apiManager) => {
  const whereClause = apiManager.whereClause;

  try {
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Reservation.update(
      { isActive: false },
      {
        where: whereClause,
        returning: true,
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeleteReservation",
      {
...
module.exports = dbScriptDeleteReservation;
