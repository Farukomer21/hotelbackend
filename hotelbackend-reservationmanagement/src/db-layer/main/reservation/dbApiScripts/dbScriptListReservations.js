...
// This script is called from the ListReservationsManager
...
const dbScriptListReservations = async (apiManager) => {
  ...
  try {
    ...
    let reservationList = await Reservation.findAll(options);

    const dbData = { items: [] };
    apiManager.reservations = [];
    if (!reservationList || reservationList.length == 0) return dbData;

    dbData.items = Array.isArray(reservationList)
      ? reservationList
      : [reservationList];
    ...
    apiManager.reservations = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListReservations",
      {
...
module.exports = dbScriptListReservations;
