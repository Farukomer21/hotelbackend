...
class DeletePackageMcpController extends PackageManagementMcpController {
  constructor(params) {
    super("deletePackage", "deletepackage", params);
    this.dataName = "package_";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeletePackageManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "deletePackage",
    description:
      "Delete a package by id (soft delete). Only for staff/internal use. Package becomes unavailable for selection on reservations.",
    parameters: DeletePackageMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new DeletePackageMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return DeletePackageMcpController.getOutputSchema().parse(result);
