...
class CreatePackageMcpController extends PackageManagementMcpController {
  constructor(params) {
    super("createPackage", "createpackage", params);
    this.dataName = "package_";
    this.crudType = "create";
  }

  createApiManager() {
    return new CreatePackageManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: CreatePackageMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new CreatePackageMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return CreatePackageMcpController.getOutputSchema().parse(result);
