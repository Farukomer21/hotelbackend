...
class UpdatePackageMcpController extends PackageManagementMcpController {
  constructor(params) {
    super("updatePackage", "updatepackage", params);
    this.dataName = "package_";
    this.crudType = "update";
  }

  createApiManager() {
    return new UpdatePackageManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: UpdatePackageMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new UpdatePackageMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return UpdatePackageMcpController.getOutputSchema().parse(result);
