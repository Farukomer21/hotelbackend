...
// This script is called from the UpdatePackageManager

const { Package_ } = require("models");

const { Package_QueryCacheInvalidator } = require("./query-cache-classes");
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.package_;
  return {
    old_package_: oldData,
    package_: newData,
...
const dbScriptUpdatePackage_ = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Package_.update(dataClause, {
      where: whereClause,
      returning: true,
    });
    ...
    const queryCacheInvalidator = new Package_QueryCacheInvalidator();
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdatePackage_",
      {
...
module.exports = dbScriptUpdatePackage_;
