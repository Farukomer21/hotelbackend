...
// This script is called from the DeletePackageManager
...
const dbScriptDeletePackage_ = async (apiManager) => {
  const whereClause = apiManager.whereClause;

  try {
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Package_.update(
      { isActive: false },
      {
        where: whereClause,
        returning: true,
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeletePackage_",
      {
...
module.exports = dbScriptDeletePackage_;
