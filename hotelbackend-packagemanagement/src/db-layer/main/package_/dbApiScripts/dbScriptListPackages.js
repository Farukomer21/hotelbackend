...
// This script is called from the ListPackagesManager
...
const dbScriptListPackages = async (apiManager) => {
  ...
  try {
    ...
    let package_List = await Package_.findAll(options);

    const dbData = { items: [] };
    apiManager.package_s = [];
    if (!package_List || package_List.length == 0) return dbData;

    dbData.items = Array.isArray(package_List) ? package_List : [package_List];
    dbData.items = dbData.items.map((item) => item.getData());

    dbData.totalRowCount = dbData.items.length;
    dbData.pageCount = 1;

    apiManager.package_s = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListPackages",
      {
...
module.exports = dbScriptListPackages;
