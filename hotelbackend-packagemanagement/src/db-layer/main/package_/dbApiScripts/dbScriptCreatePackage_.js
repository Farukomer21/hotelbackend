...
// This script is called from the CreatePackageManager
...
async function updateIfExists(dataClause) {
  const dbDoc = await Package_.findByPk(dataClause.id);
  if (dbDoc) {
...
const dbScriptCreatePackage_ = async (apiManager) => {
  ...
  try {
    ...
    if (!updated) {
      const dbDoc = await Package_.create(dataClause);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptCreatePackage_",
      {
        whereClause: null,
...
module.exports = dbScriptCreatePackage_;
