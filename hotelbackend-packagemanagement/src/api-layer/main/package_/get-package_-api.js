...
const { Package_RetrivedPublisher } = require("../../api-events/publishers");
...
const { dbScriptGetPackage_ } = require("dbLayer");

class GetPackageManager extends Package_Manager {
  ...
  async doBusiness() {
    const package_ = await dbScriptGetPackage_(this);
    return package_;
  }
  ...
  async raiseEvent() {
    Package_RetrivedPublisher.Publish(this.output, this.session).catch(
      (err) => {
