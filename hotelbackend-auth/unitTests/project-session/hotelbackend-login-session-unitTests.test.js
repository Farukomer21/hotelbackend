...
describe("HotelbackendLoginSession", () => {
  let session, mocks;
  ...
  beforeEach(() => {
    ...
    const LoginSession = proxyquire(
      "../../src/project-session/hotelbackend-login-session",
      {
        dbLayer: mocks,
        common: {
        ...
        "../../src/project-session/hotelbackend-session": class {
  ...
  describe("setLoginToRequest", () => {
    it("should set req.session and token correctly", async () => {
      ...
      expect(session.tokenName).to.equal("hotelbackend-access-token");
    });
