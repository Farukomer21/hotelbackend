...
const HotelbackendSession = require("./hotelbackend-session");

class HotelbackendLoginSession extends HotelbackendSession {
  ...
  async relogin(req) {
    console.log("hotelbackend session found but a relogin is requested");
    try {
      const userField = "id";
  ...
  async setLoginToRequest(req, byPassword, bySubject) {
    ...
    session.checkTokenMark = "hotelbackend-inapp-token";
    session._USERID = session.userId;
    ...
    const cookieName = `hotelbackend-access-token`;
...
// Export the class
module.exports = HotelbackendLoginSession;
