...
class Mobile2Factor extends VerificationServiceBase {
  constructor(req) {
    super(req, "byLink");
    this.startTopic = "hotelbackend-user-service-mobile-2FA-start";
    this.completeTopic = "hotelbackend-user-service-mobile-2FA-complete";
  }
  ...
  async start2Factor() {
    ...
    // in test mode
    result.secretCode = mobile2FA.secretCode;
  ...
  async complete2Factor() {
    ...
    if (mobile2FA.secretCode == mobile2FactorCode) {
      ...
      await setRedisData("hexasession:" + sessionId, session);
