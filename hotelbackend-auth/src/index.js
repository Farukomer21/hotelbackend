...
console.log("Starting hotelbackend-auth-service service...");
...
const HotelbackendLoginSession = require("./project-session/hotelbackend-login-session");

let processActive = false;
let expressServer = null;

const start = async () => {
  process.title = "node-hotelbackend-auth-service";
  ...
  try {
    ...
    await setRedisData("hotelbackend-auth-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key hotelbackend-auth-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "hotelbackend-auth-service is listening HTTP/REST port " +
  ...
  // init login database
  const loginSession = new HotelbackendLoginSession();
  await loginSession.init();
...
/* starting the server hotelbackend-auth-service .... */
start();
