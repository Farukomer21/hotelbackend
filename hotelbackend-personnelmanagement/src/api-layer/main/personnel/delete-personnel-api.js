...
const { PersonnelDeletedPublisher } = require("../../api-events/publishers");
...
const { dbScriptDeletePersonnel } = require("dbLayer");

class DeletePersonnelManager extends PersonnelManager {
  ...
  async doBusiness() {
    const personnel = await dbScriptDeletePersonnel(this);
    return personnel;
  }
  ...
  async raiseEvent() {
    PersonnelDeletedPublisher.Publish(this.output, this.session).catch(
      (err) => {
...
module.exports = DeletePersonnelManager;
