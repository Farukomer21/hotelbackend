...
// This script is called from the UpdatePersonnelManager
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.personnel;
  return {
    old_personnel: oldData,
    personnel: newData,
...
const dbScriptUpdatePersonnel = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Personnel.update(dataClause, {
      where: whereClause,
      returning: true,
    });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdatePersonnel",
      {
...
module.exports = dbScriptUpdatePersonnel;
