...
// This script is called from the DeletePersonnelManager
...
const dbScriptDeletePersonnel = async (apiManager) => {
  const whereClause = apiManager.whereClause;

  try {
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Personnel.update(
      { isActive: false },
      {
        where: whereClause,
        returning: true,
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeletePersonnel",
      {
...
module.exports = dbScriptDeletePersonnel;
