...
// This script is called from the ListPersonnelManager
...
const dbScriptListPersonnel = async (apiManager) => {
  ...
  try {
    ...
    let personnelList = await Personnel.findAll(options);

    const dbData = { items: [] };
    apiManager.personnels = [];
    if (!personnelList || personnelList.length == 0) return dbData;

    dbData.items = Array.isArray(personnelList)
      ? personnelList
      : [personnelList];
    ...
    apiManager.personnels = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListPersonnel",
      {
...
module.exports = dbScriptListPersonnel;
