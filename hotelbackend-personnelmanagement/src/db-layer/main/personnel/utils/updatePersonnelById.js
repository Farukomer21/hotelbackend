...
const updatePersonnelById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Personnel.findOne({
    ...
    const [rowsCount, [dbDoc]] = await Personnel.update(dataClause, options);
    if (!dbDoc) {
...
module.exports = updatePersonnelById;
