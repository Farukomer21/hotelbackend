...
const consumer = kafka.consumer({
  groupId: `hotelbackend-notification-service-push-consumer`,
});
...
const runPushSenderListener = async () => {
  ...
  await consumer.subscribe({
    topic: `hotelbackend-notification-service-notification-push`,
    fromBeginning: true,
  });
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      try {
        ...
        //**errorLog
        console.error(
          `hotelbackend-notification-service-notification-push: `,
          error,
        );
      }
    },
