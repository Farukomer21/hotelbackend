...
const consumer = kafka.consumer({
  groupId: `hotelbackend-notification-service-email-consumer`,
});
...
const runEmailSenderListener = async () => {
  ...
  await consumer.subscribe({
    topic: `hotelbackend-notification-service-notification-email`,
    fromBeginning: true,
  });
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      try {
        ...
        //**errorLog
        console.error(
          `hotelbackend-notification-service-notification-email: `,
          error,
        );
      }
    },
