...
const updateGuestById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Guest.findOne({ where: { id, isActive: true } });
    ...
    const [rowsCount, [dbDoc]] = await Guest.update(dataClause, options);
    if (!dbDoc) {
...
module.exports = updateGuestById;
