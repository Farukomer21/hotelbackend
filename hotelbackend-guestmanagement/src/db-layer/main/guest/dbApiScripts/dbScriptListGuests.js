...
// This script is called from the ListGuestsManager
...
const dbScriptListGuests = async (apiManager) => {
  ...
  try {
    ...
    let guestList = await Guest.findAll(options);

    const dbData = { items: [] };
    apiManager.guests = [];
    if (!guestList || guestList.length == 0) return dbData;

    dbData.items = Array.isArray(guestList) ? guestList : [guestList];
    dbData.items = dbData.items.map((item) => item.getData());

    dbData.totalRowCount = dbData.items.length;
    dbData.pageCount = 1;

    apiManager.guests = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListGuests",
      {
...
module.exports = dbScriptListGuests;
