...
// This script is called from the UpdateGuestManager

const { Guest } = require("models");

const { GuestQueryCacheInvalidator } = require("./query-cache-classes");
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.guest;
  return {
    old_guest: oldData,
    guest: newData,
...
const dbScriptUpdateGuest = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Guest.update(dataClause, {
      where: whereClause,
      returning: true,
    });
    ...
    const queryCacheInvalidator = new GuestQueryCacheInvalidator();
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateGuest",
      {
...
module.exports = dbScriptUpdateGuest;
