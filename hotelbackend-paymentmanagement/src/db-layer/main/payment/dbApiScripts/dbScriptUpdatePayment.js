...
// This script is called from the UpdatePaymentManager
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.payment;
  return {
    old_payment: oldData,
    payment: newData,
...
const dbScriptUpdatePayment = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Payment.update(dataClause, {
      where: whereClause,
      returning: true,
    });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdatePayment",
      {
...
module.exports = dbScriptUpdatePayment;
