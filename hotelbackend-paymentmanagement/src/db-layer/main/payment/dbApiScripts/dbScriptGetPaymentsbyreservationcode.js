...
// This script is called from the GetPaymentsByReservationCodeManager
...
const dbScriptGetPaymentsbyreservationcode = async (apiManager) => {
  ...
  try {
    ...
    let paymentList = await Payment.findAll(options);

    const dbData = { items: [] };
    apiManager.payments = [];
    if (!paymentList || paymentList.length == 0) return dbData;

    dbData.items = Array.isArray(paymentList) ? paymentList : [paymentList];
    dbData.items = dbData.items.map((item) => item.getData());

    dbData.totalRowCount = dbData.items.length;
    dbData.pageCount = 1;

    apiManager.payments = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptGetPaymentsbyreservationcode",
      {
...
module.exports = dbScriptGetPaymentsbyreservationcode;
