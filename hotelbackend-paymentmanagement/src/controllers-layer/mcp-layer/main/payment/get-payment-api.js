...
class GetPaymentMcpController extends PaymentManagementMcpController {
  constructor(params) {
    super("getPayment", "getpayment", params);
    this.dataName = "payment";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetPaymentManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "getPayment",
    description:
      "Fetches a payment record by id. For staff/internal use; not exposed to guests.",
    parameters: GetPaymentMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new GetPaymentMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return GetPaymentMcpController.getOutputSchema().parse(result);
