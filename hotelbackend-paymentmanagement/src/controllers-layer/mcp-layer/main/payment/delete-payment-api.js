...
class DeletePaymentMcpController extends PaymentManagementMcpController {
  constructor(params) {
    super("deletePayment", "deletepayment", params);
    this.dataName = "payment";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeletePaymentManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "deletePayment",
    description:
      "Deletes (soft-deletes) a payment record by ID. For staff/internal use. Payment record not physically removed if softDelete enabled.",
    parameters: DeletePaymentMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new DeletePaymentMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return DeletePaymentMcpController.getOutputSchema().parse(result);
