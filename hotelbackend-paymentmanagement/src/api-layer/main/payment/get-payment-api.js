...
const { PaymentRetrivedPublisher } = require("../../api-events/publishers");
...
const { dbScriptGetPayment } = require("dbLayer");

class GetPaymentManager extends PaymentManager {
  ...
  async doBusiness() {
    const payment = await dbScriptGetPayment(this);
    return payment;
  }
  ...
  async raiseEvent() {
    PaymentRetrivedPublisher.Publish(this.output, this.session).catch((err) => {
...
module.exports = GetPaymentManager;
