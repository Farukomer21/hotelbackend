...
The `updateSpecialRequest` Business API has 4 parameters that must be sent from the controller. Note that all parameters, except session and Redis parameters, should be provided by the client.
...
The `updateSpecialRequest` api has got 3 client parameters
