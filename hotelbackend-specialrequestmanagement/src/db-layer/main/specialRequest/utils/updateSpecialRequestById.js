...
const updateSpecialRequestById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await SpecialRequest.findOne({
    ...
    const [rowsCount, [dbDoc]] = await SpecialRequest.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateSpecialRequestById;
