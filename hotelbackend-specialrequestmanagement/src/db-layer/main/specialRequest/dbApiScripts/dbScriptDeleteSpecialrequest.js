...
// This script is called from the DeleteSpecialRequestManager
...
const dbScriptDeleteSpecialrequest = async (apiManager) => {
  const whereClause = apiManager.whereClause;

  try {
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await SpecialRequest.update(
      { isActive: false },
      {
        where: whereClause,
        returning: true,
    ...
    apiManagerspecialRequest = dbData;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeleteSpecialrequest",
      {
...
module.exports = dbScriptDeleteSpecialrequest;
