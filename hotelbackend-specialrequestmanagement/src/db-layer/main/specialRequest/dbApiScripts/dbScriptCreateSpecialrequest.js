...
// This script is called from the CreateSpecialRequestManager
...
async function updateIfExists(dataClause) {
  const dbDoc = await SpecialRequest.findByPk(dataClause.id);
  if (dbDoc) {
...
const dbScriptCreateSpecialrequest = async (apiManager) => {
  ...
  try {
    ...
    if (!updated) {
      const dbDoc = await SpecialRequest.create(dataClause);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptCreateSpecialrequest",
      {
        whereClause: null,
...
module.exports = dbScriptCreateSpecialrequest;
