...
// This script is called from the ListSpecialRequestsManager
...
const dbScriptListSpecialrequests = async (apiManager) => {
  ...
  try {
    ...
    let specialRequestList = await SpecialRequest.findAll(options);

    const dbData = { items: [] };
    apiManager.specialRequests = [];
    if (!specialRequestList || specialRequestList.length == 0) return dbData;

    dbData.items = Array.isArray(specialRequestList)
      ? specialRequestList
      : [specialRequestList];
    ...
    apiManager.specialRequests = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListSpecialrequests",
      {
...
module.exports = dbScriptListSpecialrequests;
