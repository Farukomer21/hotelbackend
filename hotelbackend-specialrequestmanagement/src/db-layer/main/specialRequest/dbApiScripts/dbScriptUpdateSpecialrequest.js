...
// This script is called from the UpdateSpecialRequestManager
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.specialRequest;
  return {
    old_specialRequest: oldData,
    specialRequest: newData,
...
const dbScriptUpdateSpecialrequest = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await SpecialRequest.update(dataClause, {
      where: whereClause,
      returning: true,
    });
    ...
    apiManager.specialRequest = dbData;
    ...
    const queryCacheInvalidator = new SpecialRequestQueryCacheInvalidator();
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateSpecialrequest",
      {
...
module.exports = dbScriptUpdateSpecialrequest;
