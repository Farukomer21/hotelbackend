...
const { dbScriptGetSpecialrequest } = require("dbLayer");

class GetSpecialRequestManager extends SpecialRequestManager {
  ...
  async doBusiness() {
    const specialrequest = await dbScriptGetSpecialrequest(this);
  ...
  async raiseEvent() {
    SpecialrequestRetrivedPublisher.Publish(this.output, this.session).catch(
      (err) => {
...
module.exports = GetSpecialRequestManager;
