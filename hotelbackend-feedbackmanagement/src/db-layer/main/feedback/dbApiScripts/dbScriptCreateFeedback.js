...
// This script is called from the CreateFeedbackManager
...
async function updateIfExists(dataClause) {
  const dbDoc = await Feedback.findByPk(dataClause.id);
  if (dbDoc) {
...
const dbScriptCreateFeedback = async (apiManager) => {
  ...
  try {
    ...
    if (!updated) {
      const dbDoc = await Feedback.create(dataClause);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptCreateFeedback",
      {
        whereClause: null,
...
module.exports = dbScriptCreateFeedback;
