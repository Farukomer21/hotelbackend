...
// This script is called from the UpdateFeedbackManager

const { Feedback } = require("models");

const { FeedbackQueryCacheInvalidator } = require("./query-cache-classes");
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.feedback;
  return {
    old_feedback: oldData,
    feedback: newData,
...
const dbScriptUpdateFeedback = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Feedback.update(dataClause, {
      where: whereClause,
      returning: true,
    });
    ...
    const queryCacheInvalidator = new FeedbackQueryCacheInvalidator();
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateFeedback",
      {
...
module.exports = dbScriptUpdateFeedback;
