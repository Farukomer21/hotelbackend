...
// This script is called from the ListFeedbacksManager
...
const dbScriptListFeedbacks = async (apiManager) => {
  ...
  try {
    ...
    let feedbackList = await Feedback.findAll(options);

    const dbData = { items: [] };
    apiManager.feedbacks = [];
    if (!feedbackList || feedbackList.length == 0) return dbData;

    dbData.items = Array.isArray(feedbackList) ? feedbackList : [feedbackList];
    dbData.items = dbData.items.map((item) => item.getData());

    dbData.totalRowCount = dbData.items.length;
    dbData.pageCount = 1;

    apiManager.feedbacks = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListFeedbacks",
      {
...
module.exports = dbScriptListFeedbacks;
