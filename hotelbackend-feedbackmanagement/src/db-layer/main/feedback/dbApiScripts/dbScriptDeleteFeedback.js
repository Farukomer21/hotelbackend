...
// This script is called from the DeleteFeedbackManager
...
const dbScriptDeleteFeedback = async (apiManager) => {
  const whereClause = apiManager.whereClause;

  try {
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Feedback.update(
      { isActive: false },
      {
        where: whereClause,
        returning: true,
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeleteFeedback",
      {
...
module.exports = dbScriptDeleteFeedback;
