...
class DeleteFeedbackMcpController extends FeedbackManagementMcpController {
  constructor(params) {
    super("deleteFeedback", "deletefeedback", params);
    this.dataName = "feedback";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteFeedbackManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "deleteFeedback",
    description:
      "Delete (soft-delete) a feedback entry by its ID. For internal moderation or error correction only.",
    parameters: DeleteFeedbackMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new DeleteFeedbackMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return DeleteFeedbackMcpController.getOutputSchema().parse(result);
