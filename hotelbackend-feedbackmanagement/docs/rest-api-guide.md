...
- **Preview:** `https://hotelbackend.prw.mindbricks.com/feedbackManagement-api`
- **Staging:** `https://hotelbackend-stage.mindbricks.co/feedbackManagement-api`
- **Production:** `https://hotelbackend.mindbricks.co/feedbackManagement-api`
...
By utilizing these common parameters, you can tailor the behavior of API requests to suit your specific requirements, ensuring optimal performance and usability of the `FeedbackManagement` service.
...
_Resource Definition_ : Feedback submitted by guests for a hotel stay or service, linked to a reservation. Contains rating, comments, staff response, category, and submission timestamp.
_Feedback Resource Properties_
| Name | Type | Required | Default | Definition |
| ---- | ---- | -------- | ------- | ---------- |
| **reservationId** | ID | | | _Reference to the reservation for which this feedback is submitted. Foreign key to reservationManagement:reservation._ |
| **rating** | Integer | | | _Feedback rating from 1 (worst) to 5 (best). Required._ |
| **comment** | Text | | | _Guest&#39;s free-form comment, experience note, or suggestion. Required for meaningful feedback._ |
| **submittedAt** | Date | | | _Date/time the feedback was submitted. Auto-set to now on create if not provided._ |
| **response** | Text | | | _Optional staff comment or follow-up in response to feedback. Editable only by staff via updateFeedback._ |
| **category** | String | | | _Feedback category or topic for staff analysis/filtering (e.g., housekeeping, room, service). Required. If not selected by guest, default to &#39;general&#39;._ |

## Business Api

### Create Feedback API

_API Definition_ : Submit new guest feedback for a reservation. Sets submittedAt to now if not provided. Checks valid rating (1-5) and that reservationId exists.

_API Crud Type_ : create

_Default access route_ : _POST_ `/v1/feedbacks`

#### Parameters

The createFeedback api has got 5 parameters
...
> For a detailed description of the `` api with its internal and inter-service logic please refer to the [Business API Specification Document For Create Feedback](businessApi/createFeedback).

### Get Feedback API

_API Definition_ : Get feedback entry by its unique ID. Used by staff/internal tools for review or follow-up.

_API Crud Type_ : get

_Default access route_ : _GET_ `/v1/feedbacks/:feedbackId`
...
_API Definition_ : Staff/internal tool API to update feedback entry (response, category, comment, rating). reservationId and submittedAt cannot be updated.

_API Crud Type_ : update

_Default access route_ : _PATCH_ `/v1/feedbacks/:feedbackId`

#### Parameters

The updateFeedback api has got 5 parameters
...
_API Definition_ : Delete (soft-delete) a feedback entry by its ID. For internal moderation or error correction only.

_API Crud Type_ : delete

_Default access route_ : _DELETE_ `/v1/feedbacks/:feedbackId`
...
> For a detailed description of the `` api with its internal and inter-service logic please refer to the [Business API Specification Document For Delete Feedback](businessApi/deleteFeedback).

### List Feedbacks API

_API Definition_ : List all feedback entries with support for filtering by reservation, category, rating, date. Internal use for staff to organize/analyze feedback records.

_API Crud Type_ : list

_Default access route_ : _GET_ `/v1/feedbacks`
