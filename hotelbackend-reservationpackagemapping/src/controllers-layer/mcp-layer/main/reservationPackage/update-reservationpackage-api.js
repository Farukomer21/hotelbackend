...
class UpdateReservationPackageMcpController extends ReservationPackageMappingMcpController {
  constructor(params) {
    super("updateReservationPackage", "updatereservationpackage", params);
    this.dataName = "reservationPackage";
    this.crudType = "update";
  }

  createApiManager() {
    return new UpdateReservationPackageManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Update a reservation-package mapping (assignedAt). Does NOT allow changing reservationId or packageId to avoid reassignment; only assignedAt is permitted.",
    parameters: UpdateReservationPackageMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new UpdateReservationPackageMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return UpdateReservationPackageMcpController.getOutputSchema().parse(result);
