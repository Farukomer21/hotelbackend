...
const { dbScriptGetReservationpackage } = require("dbLayer");

class GetReservationPackageManager extends ReservationPackageManager {
  ...
  async doBusiness() {
    const reservationpackage = await dbScriptGetReservationpackage(this);
  ...
  async raiseEvent() {
    ReservationpackageRetrivedPublisher.Publish(
      this.output,
      this.session,
    ).catch((err) => {
...
module.exports = GetReservationPackageManager;
