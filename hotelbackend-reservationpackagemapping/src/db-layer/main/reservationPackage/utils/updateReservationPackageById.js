...
const updateReservationPackageById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await ReservationPackage.findOne({
    ...
    const [rowsCount, [dbDoc]] = await ReservationPackage.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateReservationPackageById;
