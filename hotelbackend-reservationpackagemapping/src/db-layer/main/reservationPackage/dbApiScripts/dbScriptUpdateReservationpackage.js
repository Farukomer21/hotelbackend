...
// This script is called from the UpdateReservationPackageManager
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.reservationPackage;
  return {
    old_reservationPackage: oldData,
    reservationPackage: newData,
...
const dbScriptUpdateReservationpackage = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await ReservationPackage.update(dataClause, {
      where: whereClause,
      returning: true,
    });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateReservationpackage",
      {
...
module.exports = dbScriptUpdateReservationpackage;
