...
// This script is called from the ListReservationPackagesManager
...
const dbScriptListReservationpackages = async (apiManager) => {
  ...
  try {
    ...
    let reservationPackageList = await ReservationPackage.findAll(options);

    const dbData = { items: [] };
    apiManager.reservationPackages = [];
    if (!reservationPackageList || reservationPackageList.length == 0)
      return dbData;

    dbData.items = Array.isArray(reservationPackageList)
    ...
    apiManager.reservationPackages = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListReservationpackages",
      {
...
module.exports = dbScriptListReservationpackages;
