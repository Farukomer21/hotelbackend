...
// This script is called from the DeleteReservationPackageManager
...
const dbScriptDeleteReservationpackage = async (apiManager) => {
  const whereClause = apiManager.whereClause;

  try {
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await ReservationPackage.update(
      { isActive: false },
      {
        where: whereClause,
        returning: true,
    ...
    apiManagerreservationPackage = dbData;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeleteReservationpackage",
      {
...
module.exports = dbScriptDeleteReservationpackage;
