...
const { RoompriceRetrivedPublisher } = require("../../api-events/publishers");
...
const { dbScriptGetRoomprice } = require("dbLayer");

class GetRoomPriceManager extends RoomPriceManager {
  ...
  async doBusiness() {
    const roomprice = await dbScriptGetRoomprice(this);
    return roomprice;
  }
  ...
  async raiseEvent() {
    RoompriceRetrivedPublisher.Publish(this.output, this.session).catch(
      (err) => {
...
module.exports = GetRoomPriceManager;
