...
// This script is called from the DeleteRoomPriceManager
...
const dbScriptDeleteRoomprice = async (apiManager) => {
  const whereClause = apiManager.whereClause;

  try {
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await RoomPrice.update(
      { isActive: false },
      {
        where: whereClause,
        returning: true,
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeleteRoomprice",
      {
...
module.exports = dbScriptDeleteRoomprice;
