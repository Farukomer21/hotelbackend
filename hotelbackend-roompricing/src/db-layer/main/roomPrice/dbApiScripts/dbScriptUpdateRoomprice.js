...
// This script is called from the UpdateRoomPriceManager
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.roomPrice;
  return {
    old_roomPrice: oldData,
    roomPrice: newData,
...
const dbScriptUpdateRoomprice = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await RoomPrice.update(dataClause, {
      where: whereClause,
      returning: true,
    });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateRoomprice",
      {
...
module.exports = dbScriptUpdateRoomprice;
