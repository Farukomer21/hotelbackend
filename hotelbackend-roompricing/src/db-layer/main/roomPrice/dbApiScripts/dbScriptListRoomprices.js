...
// This script is called from the ListRoomPricesManager
...
const dbScriptListRoomprices = async (apiManager) => {
  ...
  try {
    ...
    let roomPriceList = await RoomPrice.findAll(options);

    const dbData = { items: [] };
    apiManager.roomPrices = [];
    if (!roomPriceList || roomPriceList.length == 0) return dbData;

    dbData.items = Array.isArray(roomPriceList)
      ? roomPriceList
      : [roomPriceList];
    ...
    apiManager.roomPrices = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListRoomprices",
      {
...
module.exports = dbScriptListRoomprices;
