...
By utilizing these common parameters, you can tailor the behavior of API requests to suit your specific requirements, ensuring optimal performance and usability of the `RoomPricing` service.
...
_Resource Definition_ : Represents a price entry for a hotel room, including validity period and history/audit reason. Linked to a roomInventory:room. Used to track price changes over time for reporting, billing, and audit.
_RoomPrice Resource Properties_
| Name | Type | Required | Default | Definition |
| ---- | ---- | -------- | ------- | ---------- |
| **roomId** | ID | | | _Foreign key to roomInventory:room. Identifies which room this price applies to._ |
| **price** | Double | | | _Room price in base hotel currency. Must be &gt;= 0._ |
| **validFrom** | Date | | | _Start date/time for this price entry. Inclusive. Required for all entries._ |
| **validTo** | Date | | | _End date/time for this price entry. Inclusive. Required for all entries._ |
| **reason** | String | | | _(Optional) Free-text reason describing why the price was set or modified (e.g., &#39;seasonal adjustment&#39;, &#39;promo&#39;, &#39;rate update&#39;)._ |

## Business Api

### Create Roomprice API

_API Definition_ : Creates a new room price record for a specific room. Requires valid roomId, positive price, and a non-overlapping date range. Stores optional reason for audit/change history. Only staff/internal use.

_API Crud Type_ : create

_Default access route_ : _POST_ `/v1/roomprices`

#### Parameters

The createRoomPrice api has got 5 parameters
...
_API Definition_ : Fetch a room price record by its unique ID. Used for internal/staff lookup or history review.

_API Crud Type_ : get

_Default access route_ : _GET_ `/v1/roomprices/:roomPriceId`
...
_API Definition_ : Update an existing room price record (by ID). Validates positive price, date range, and non-overlapping for the room. Only price, validFrom/validTo, and reason are allowed to change. Internal/staff only.

_API Crud Type_ : update

_Default access route_ : _PATCH_ `/v1/roomprices/:roomPriceId`

#### Parameters

The updateRoomPrice api has got 5 parameters
...
| validTo     | Date   | false    | request.body?.validTo       |
| reason      | String | false    | request.body?.reason        |
...
_API Definition_ : Soft-deletes a room price record by its ID. Used to maintain audit/history and enable staff to remove obsolete or incorrect entries.

_API Crud Type_ : delete

_Default access route_ : _DELETE_ `/v1/roomprices/:roomPriceId`
...
> For a detailed description of the `` api with its internal and inter-service logic please refer to the [Business API Specification Document For Delete Roomprice](businessApi/deleteRoomPrice).

### List Roomprices API

_API Definition_ : List all room price records, supporting filter by roomId, validFrom, validTo, and full audit/history review. Used by staff/internal tools to view price history, current rates, or for reporting.

_API Crud Type_ : list

_Default access route_ : _GET_ `/v1/roomprices`
