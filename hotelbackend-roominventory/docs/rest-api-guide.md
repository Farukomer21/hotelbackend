...
By utilizing these common parameters, you can tailor the behavior of API requests to suit your specific requirements, ensuring optimal performance and usability of the `RoomInventory` service.
...
_Resource Definition_ : Represents a hotel room and its specifications for assignment to reservations. Includes type, amenities, floor, status, occupancy, and description.
_Room Resource Properties_
| Name | Type | Required | Default | Definition |
| ---- | ---- | -------- | ------- | ---------- |
| **roomNumber** | String | | | _The unique identifier for the room within the hotel (e.g., &#39;101&#39;, &#39;A04&#39;)._ |
| **type** | Enum | | | _Type or category of the room (e.g., single, double, suite, deluxe, family, accessible, other)._ |
| **amenities** | String | | | _List of amenities and facilities available in this room (e.g., &#39;wifi&#39;, &#39;mini-bar&#39;, &#39;tv&#39;)._ |
| **floor** | Integer | | | _The floor on which the room is located._ |
| **description** | Text | | | _Freeform description for the room (location, view, remarks for staff) for reference in chatbots and tools._ |
| **occupancyLimit** | Integer | | | _Maximum number of guests allowed in this room._ |
| **status** | Enum | | | _Operational status of the room: available (can assign), occupied (assigned to reservation), maintenance, or outOfService._ |
...
| **outOfService** | `"outOfService""` | 3 |

## Business Api

### Create Room API

_API Definition_ : Create a new room entry. RoomNumber must be unique. Type and status must be valid enums. Amenities optional. For staff use or integrations.

_API Crud Type_ : create

_Default access route_ : _POST_ `/v1/rooms`

#### Parameters

The createRoom api has got 7 parameters
...
> For a detailed description of the `` api with its internal and inter-service logic please refer to the [Business API Specification Document For Create Room](businessApi/createRoom).

### Update Room API

_API Definition_ : Update a room entry by id. All room fields except id can be modified. Enforces uniqueness, valid enums, and positive values. Used by staff/integrations.
...
The updateRoom api has got 8 parameters
...
_API Definition_ : Delete a room entry by id. Uses soft delete (isActive=false) for referential integrity with reservations. For use by staff/integrators.
...
_API Definition_ : Fetch detailed info for a room by id. Used by staff tools and for assignment in reservation creation.
...
_API Definition_ : List and filter all hotel rooms. Supports filters for roomNumber, type, status, floor, and occupancyLimit. Returns for internal tools and assignment to reservation.
