...
const { RoomDeletedPublisher } = require("../../api-events/publishers");
...
const { dbScriptDeleteRoom } = require("dbLayer");

class DeleteRoomManager extends RoomManager {
  ...
  async doBusiness() {
    const room = await dbScriptDeleteRoom(this);
    return room;
  }
  ...
  async raiseEvent() {
    RoomDeletedPublisher.Publish(this.output, this.session).catch((err) => {
...
module.exports = DeleteRoomManager;
