...
// This script is called from the ListRoomsManager
...
const dbScriptListRooms = async (apiManager) => {
  ...
  try {
    ...
    let roomList = await Room.findAll(options);

    const dbData = { items: [] };
    apiManager.rooms = [];
    if (!roomList || roomList.length == 0) return dbData;

    dbData.items = Array.isArray(roomList) ? roomList : [roomList];
    dbData.items = dbData.items.map((item) => item.getData());

    dbData.totalRowCount = dbData.items.length;
    dbData.pageCount = 1;

    apiManager.rooms = dbData.items;
    ...
    console.log(err);
    //**errorLog
    throw new HttpServerError("errMsg_dbErrorWhenExecuting_dbScriptListRooms", {
...
module.exports = dbScriptListRooms;
