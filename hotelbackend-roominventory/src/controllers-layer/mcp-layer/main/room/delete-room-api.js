...
class DeleteRoomMcpController extends RoomInventoryMcpController {
  constructor(params) {
    super("deleteRoom", "deleteroom", params);
    this.dataName = "room";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteRoomManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "deleteRoom",
    description:
      "Delete a room entry by id. Uses soft delete (isActive=false) for referential integrity with reservations. For use by staff/integrators.",
    parameters: DeleteRoomMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new DeleteRoomMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return DeleteRoomMcpController.getOutputSchema().parse(result);
